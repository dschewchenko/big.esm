...
import { absBig } from "../utils/absBig"; // Assuming absBig is here
import { compareBig } from "../utils/compareBig"; // Assuming compareBig is here
...
export function sinBig(angle: Big, precision: number = DEFAULT_SIN_PRECISION): Big {
    ...
    // This is essentially a modulo operation: reducedAngle = angle % TWO_PI_BIG
    // divBig result is integer, so we can do: angle - floor(angle / TWO_PI_BIG) * TWO_PI_BIG
    if (compareBig(reducedAngle, TWO_PI_BIG) >= 0) {
        const revolutions = divBig(reducedAngle, TWO_PI_BIG, 0); // Integer division
    ...
    // Note: angle reduction should be extremely precise. Using PI_BIG, TWO_PI_BIG from constants.
