...
describe("cosBig", () => {
    ...
    it("should calculate cos(PI/2) correctly (should be 0)", () => {
        checkCosNearZero(PI_HALF_BIG_TEST, defaultPrecision);
    });

    it("should calculate cos(PI) correctly", () => {
        checkCos(PI_BIG_TEST, "-1.0", defaultPrecision, 5);
    });
    ...
    it("should calculate cos(2*PI) correctly (should be 1)", () => {
        ...
        // checkCos(TWO_PI_BIG_TEST, "1.0", defaultPrecision, 5);
        const result = cosBig(TWO_PI_BIG_TEST, defaultPrecision);
    ...
    it("should calculate cos(PI/3) correctly (0.5)", () => {
        const piOverThree = divBig(PI_BIG_TEST, createBig(3), defaultPrecision + 5);
        checkCos(piOverThree, "0.5", defaultPrecision, 4); // Compare "0.500"
    });

    it("should calculate cos(-PI) correctly", () => {
        const negPi = cloneBig(PI_BIG_TEST);
    ...
    it("should maintain precision", () => {
        const angle = divBig(PI_BIG_TEST, createBig(6), 35); // PI/6, cos(PI/6) = sqrt(3)/2 ~ 0.86602540378
